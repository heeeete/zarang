...
export const ExplorePostCard = ({ post }: ExplorePostCardProps) => {
  ...
  return (
    <div className="flex w-full flex-col gap-2 pb-4">
      ...
      >
        {optimizedImage ? (
          <div style={{ paddingBottom: `${(1 / aspectRatio) * 100}%` }} className="relative w-full">
            ...
            {post.audio_url && (
              <div className="absolute top-2 left-2 z-10 rounded-full bg-black/40 p-1.5 text-white backdrop-blur-sm shadow-sm">
      ...
      {post.title && (
        <div className="px-1">
          <h4 className="line-clamp-2 text-[13px] font-bold leading-snug tracking-tight text-neutral-800">
            {post.title}
          </h4>
        </div>
      )}
